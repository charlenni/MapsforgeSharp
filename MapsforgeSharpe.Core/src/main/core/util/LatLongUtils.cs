...
namespace org.mapsforge.core.util
{
	...
	public sealed class LatLongUtils
	{
		...
		public static double[] ParseCoordinateString(string coordinatesString, int numberOfCoordinates)
		{
			...
			if (isDelimiter)
			{
				throw new System.ArgumentException("invalid coordinate delimiter: " + coordinatesString);
			}
			else if (tokens.Count != numberOfCoordinates)
			{
			...
			{
				coordinates[i] = double.Parse(tokens[i]);
			}
			return coordinates;
		}

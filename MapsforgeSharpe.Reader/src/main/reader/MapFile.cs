...
namespace org.mapsforge.map.reader
{
    ...
    using MapDataStore = org.mapsforge.map.datastore.MapDataStore;
    using MapReadResult = org.mapsforge.map.datastore.MapReadResult;
    using PoiWayBundle = org.mapsforge.map.datastore.PoiWayBundle;
    using PointOfInterest = org.mapsforge.map.datastore.PointOfInterest;
    using Way = org.mapsforge.map.datastore.Way;
    using MapFileException = org.mapsforge.map.reader.header.MapFileException;
    using MapFileHeader = org.mapsforge.map.reader.header.MapFileHeader;
    ...
    public class MapFile : MapDataStore
	{
		...
		public MapFile(IFile mapFile, string language) : base(language)
		{
			...
			try
			{
                var folder = PCLStorage.FileSystem.Current.GetFolderFromPathAsync(mapFile.Path).Result;
                var existenceResult = folder.CheckExistsAsync(mapFile.Name).Result;
                Stream stream;
                ...
                else if (existenceResult != ExistenceCheckResult.FileExists)
                {
        ...
        public MapFile(string mapFileName, string language) : this(PCLStorage.FileSystem.Current.GetFileFromPathAsync(mapFileName).Result, language)
		{
		}

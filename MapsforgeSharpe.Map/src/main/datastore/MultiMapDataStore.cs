...
namespace org.mapsforge.map.datastore
{
    using System;
	...
	public class MultiMapDataStore : MapDataStore
	{
		...
		public override long GetDataTimestamp(Tile tile)
		{
			...
			{
				case org.mapsforge.map.datastore.MultiMapDataStore.DataPolicy.RETURN_FIRST:
					foreach (MapDataStore mdb in mapDatabases)
					{
					...
					return 0;
				case org.mapsforge.map.datastore.MultiMapDataStore.DataPolicy.RETURN_ALL:
				case org.mapsforge.map.datastore.MultiMapDataStore.DataPolicy.DEDUPLICATE:
					long result = 0;
					foreach (MapDataStore mdb in mapDatabases)
					{
						if (mdb.SupportsTile(tile))
